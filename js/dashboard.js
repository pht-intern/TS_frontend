...
// Always call backend logout first so server session ends (fixes "session doesn't end" for app@...)
...
async function authenticatedFetch(url, options = {}) {
    ...
    try {
        ...
        // 401 = session expired → logout and redirect to login
        if (response.status === 401) {
            console.warn('authenticatedFetch got 401 for', url);
            redirectToLoginOnSessionExpired();
            throw new Error('Session expired. Please log in again.');
        }
        // 403 = forbidden → do NOT clear session; redirect to home
...
function initDashboard() {
    ...
    const residentialGalleryContainer = document.getElementById('residentialGalleryContainer');
    if (residentialGalleryContainer) {
        residentialGalleryContainer.addEventListener('input', function (e) {
            const titleInput = e.target;
            if (titleInput && titleInput.getAttribute('name') === 'gallery_image_title') {
                const item = titleInput.closest('.dashboard-gallery-item');
                if (item) {
                    const titleSpan = item.querySelector('.dashboard-gallery-item-title');
                    const items = residentialGalleryContainer.querySelectorAll('.dashboard-gallery-item');
...
async function handleLogout() {
    ...
    if (confirmed) {
        ...
        try {
            ...
            localStorage.removeItem('_session_restore');
        } catch (e) { /* ignore */ }
...
function renderProperties(properties) {
    ...
    if (properties.length === 0) {
        tbody.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 2rem;">No properties found</td></tr>';
        return;
    }

    // Format timestamp from API (ISO string or Date) for display
    ...
    tbody.innerHTML = properties.map(property => {
        ...
        if (!property) {
            ...
            return '<tr><td colspan="10" style="text-align: center; padding: 2rem; color: #ef4444;">Invalid property data</td></tr>';
        }
        ...
        <tr>
            ...
            <td>
                <span class="dashboard-table-category">${escapeHtml(categoryDisplay)}</span>
            </td>
            <td>
                <span class="dashboard-table-timestamp">${formatTimestamp(property.created_at)}</span>
            </td>
            <td>
...
function resetResidentialPropertyModalState() {
    ...
    if (form) {
        ...
        if (projectCategoryCacheInput) {
            projectCategoryCacheInput.value = '';
        }
...
function validateResidentialPropertyStep(stepNumber) {
    if (stepNumber === 1) {
        ...
        const projectCategory = document.getElementById('residentialProjectCategory');
        if (!projectCategory || !projectCategory.value) {
            showNotification('Please select project category', 'error');
    ...
    if (stepNumber === 2) {
        ...
        if (propertyType === 'apartments') {
            ...
            // Commercial: project name required (already validated above)
        } else {
    ...
    if (stepNumber === 3) {
        ...
        if (!galleryItems.length) {
            ...
            document.getElementById('residentialAddImageBtn')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
            return false;
        }
        for (let i = 0; i < galleryItems.length; i++) {
            const item = galleryItems[i];
            const imageNum = i + 1;
            const hasImage = item.querySelector('input[name="gallery_image_url"]')?.value?.trim() ||
                item.querySelector('.dashboard-gallery-item-image img');
            if (!hasImage) {
                showNotification('Please upload or select an image for Image ' + imageNum + '.', 'error');
                item.scrollIntoView({ behavior: 'smooth', block: 'center' });
                return false;
            }
            const titleInput = item.querySelector('input[name="gallery_image_title"]');
            if (!titleInput || !titleInput.value.trim()) {
                showNotification('Please enter image title for Image ' + imageNum + '.', 'error');
                titleInput?.focus();
                return false;
            }
...
function populatePropertyTypeByProjectCategory() {
    ...
    const category = (projectCategorySelect && projectCategorySelect.value) ? projectCategorySelect.value.toLowerCase() : 'residential';
    const types = category === 'commercial' ? COMMERCIAL_PROPERTY_TYPES : RESIDENTIAL_PROPERTY_TYPES;
...
async function loadCategoriesForPropertyTypeDropdown() {
    try {
        // Fetch API categories for other uses (e.g. allCategories)
        const timestamp = new Date().getTime();
        ...
        if (response.ok) {
            ...
            if (data.categories && Array.isArray(data.categories)) {
                ...
                allCategories.sort((a, b) => (a.display_name || '').localeCompare(b.display_name || ''));
            }
        }
    } catch (e) {
        // ignore
    }
    // Populate Property Type dropdown from Project Category (residential / commercial)
    populatePropertyTypeByProjectCategory();
}
...
async function handleResidentialPropertySubmit(e) {
    ...
    const data = {
        ...
        project_category: formData.get('project_category') || 'residential', // Residential | Commercial
        ...
        possession_date: formData.get('possession_date') || null,
        is_featured: false,
        is_active: true,
        images: [],
        features: []
...
// Add new gallery item (category = '' for new rows so user must "Select a category")
function addResidentialGalleryItem(imageUrl = null, title = '', category = '') {
    const galleryContainer = document.getElementById('residentialGalleryContainer');
    ...
    const displayTitle = (title && String(title).trim()) ? String(title).trim().replace(/</g, '&lt;').replace(/>/g, '&gt;') : ('Image ' + (galleryContainer.children.length + 1));
    const galleryItemHTML = `
        <div class="dashboard-gallery-item" data-item-id="${itemId}" ${dataImageUrlAttr}>
            <div class="dashboard-gallery-item-header">
                <span class="dashboard-gallery-item-title">${displayTitle}</span>
            ...
            <div class="dashboard-gallery-item-content">
                <div class="dashboard-gallery-item-image${imageUrlString ? ' has-image' : ''}">
                    ${imageUrlString ? 
                        `<img src="${imageUrlString}" alt="No image selected" loading="lazy" style="width: 100%; height: auto; border-radius: 8px;" onerror="this.parentElement.classList.remove('has-image'); this.outerHTML='<div class=&quot;dashboard-gallery-item-image-placeholder&quot;><i class=&quot;fas fa-image&quot;></i><p>No image selected</p></div>'">` : 
                ...
                <div class="dashboard-gallery-item-fields">
                    ...
                    <div class="dashboard-form-group">
                        ...
                        <input type="text" name="gallery_image_title" placeholder="e.g., Living Room View" value="${String(title || '').replace(/"/g, '&quot;').replace(/'/g, '&#39;')}" required>
                    </div>
                    <div class="dashboard-form-group">
                        <label>
...
// Update gallery item numbers and display titles (use Image Title input when given, else "Image N")
function updateGalleryItemNumbers() {
    ...
    items.forEach((item, index) => {
        ...
        if (titleElement) {
            ...
            titleElement.textContent = customTitle || ('Image ' + (index + 1));
        }
    });
}
...
async function handleResidentialGalleryImageUpload(itemId, fileInput) {
    ...
    try {
        // Require category to be selected before uploading (no default)
        const categorySelect = item.querySelector('select[name="gallery_image_category"]');
        const imageCategory = categorySelect ? categorySelect.value : '';
        if (!imageCategory || !imageCategory.trim()) {
            showNotification('Please select an image category before uploading.', 'error');
            item.dataset.uploading = 'false';
            delete item.dataset.lastUploadToken;
        ...
        // Update the image preview with the uploaded URL (add has-image so styling matches Image 1)
        imageContainer.classList.add('has-image');
        imageContainer.innerHTML = `<img src="${imageUrl}" alt="No image selected" loading="lazy" style="width: 100%; height: auto; border-radius: 8px;" onerror="this.parentElement.classList.remove('has-image'); this.outerHTML='<div class=&quot;dashboard-gallery-item-image-placeholder&quot;><i class=&quot;fas fa-image&quot;></i><p>No image selected</p></div>'">`;
